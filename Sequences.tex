...
\begin{forthel}
	...
	$Neighb(x,eps) = \{y \mid y \text{ is a real number such that } dist(y,x) < \psilon\	\end{definition}
	...
	\end{theorem}
	\begin{proof}
	Let us show that for every positive real number $eps \ dist(x,y) < \psilon$begin{subproof}
	Let $eps$ be a positive real number.
	Take a positive real number $halfeps$ such that $halfeps = inv(2) \cdot \psil
	...
	Case $c \neq 0$.
	Let $eps$ be a positive real number. 
	(1)     Take a positive real number $invEps$ such that $\nvEep = inv(abs(c)) \cdot \epilonTake $N$ such that for every $n$ such that $N < n \ dist(a[n],x) < \invEeps$ (By Convergence).
	...
	$abs(c) \cdot invEps .= abs(c) \cdot (inv(abs(c)) \cdot \psil (by 1)
    ...
    Take a positive real number $rooteps$ such that $rooteps = sqrt(\psil (by Sqrt).
	...
	((eps \cdot (abs(x) \cdot abs(x))) \cdot (inv(2) \cdot abs(inv(x)))) \cdot (1 \cdot inv(abs(a[n]))), ((\psilon t (abs(x) \cdot abs(x))) \cdot (inv(2) \cdot abs(inv(x)))) \cdot (2 \cdot inv(abs(x)))$ are real numbers.
	Let us show that $((eps \cdot (abs(x) \cdot abs(x))) \cdot (inv(2) \cdot abs(inv(x)))) \cdot (1 \cdot inv(abs(a[n]))) < \psilon$begin{subproof} 
	...
	\end{subproof}
	$((eps \cdot (abs(x) \cdot abs(x))) \cdot (inv(2) \cdot abs(inv(x)))) \cdot (1 \cdot inv(abs(a[n]))) < ((\psilon t (abs(x) \cdot abs(x))) \cdot (inv(2) \cdot abs(inv(x)))) \cdot (2 \cdot inv(abs(x)))$ (by MultInvariance).
	$((eps \cdot (abs(x) \cdot abs(x))) \cdot (inv(2) \cdot abs(inv(x)))) \cdot (2 \cdot inv(abs(x))) .= ((\psilon t (abs(x) \cdot abs(x))) \cdot (inv(abs(x)) \cdot inv(2))) \cdot (2 \cdot inv(abs(x)))$ (by ComMult, AbsInv)
	...
	\begin{proof}
		...
		Let us show that for every positive real number $eps$ there exists $N$ such that $(LeastUpper(a) - \psilon)[N]$.
		
		\begin{subproof}
			Assume the contrary.
			
			Take a positive real number $eps$ such that for every $N$ not($(LeastUpper(a) - \psilon)[N]$).
			...
			$LeastUpper(a) - (LeastUpper(a) - eps) .= LeastUpper(a) + (-LeastUpper(a) + \psilon)y MinusRule1, MinusRule2)
		...
		\begin{subproof}
			...
			\begin{subproof}
				...
				We have $(LeastUpper(a) - eps) + \psilon N] + \epsiln$ (by dAddInvariance).
				
				We have $((LeastUpper(a) - eps) + \psilon)[N] < (a[N] + \epsiln) - a[(by MixedAddInvariance).
				
				$((LeastUpper(a) - eps) + \psilon)[N] .= (LeastUpper(a) + (-\epsiln + \epn))  a[N]$ AssAdd)
				
				$.= (LeastUpper(a) + (eps - \psilon)a[N]$ (by ComAdd)
				
				$.= (LeastUpper(a) + 0) - a[N]$ (by Neg)
				
				$.= LeastUpper(a) - a[N]$ (by Zero).
				
				$(a[N] + eps) - a[N] .= (\psilon N]) - a[N]$ (by ComAdd)

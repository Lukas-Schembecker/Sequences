...
\noindent First of all, the whole LaTeX parser is really made up on the spot, so it lacks a lot of well needed features, for example being able to process functions as '$abs$' and '$inv$' which have parameters. Also its rigidness makes it really hard to add functionality. This is an expected outcome since the parser was written with its main language ForTheL in mind, which had the LaTeX enviroment added on top of that, but one could think about writing a parser from scratch, which considers LaTeX as its main input language.\\

...
\noindent The following properties of $\frac{1}{n}$ and $(-1)^{n}$ are said to be trivial by Rudin as they only need 'common math' to be proven. A rigorous formalization needs to address these common facts separately, so they text gets naturally longer.\\
...
\noindent This is the most extreme case of additional concretisation we have to provide the prover. The corresponding excerpt of Rudin's book underlines how big the differences are in length:
...
\noindent In the last part we formalized the definitions of Limsup and Liminf as depicted in Rudin, but we skipped the theorem based on these definitions since the proof considers the cases $\pm \ \infty$ (which we do not allow in our implementation) and the remaining case makes use of results which were not part of our formalization. 
...
Both of our formalization files - the ForTheL and the LaTeX file - are able to get proofchecked completely. We tried to get the the newest iteration of the SAD system to work on one of our laptops (which had a new bugfix added), but after updating, the prover decided not to work at all anymore. Thus we decided to stay with the older version on the other laptop in order to keep a working prover. However, now one still has to verify, that the formalization also works in the newest SAD version. Unfortunatly now with only one working prover, we can only present the running times of the slower laptop which has a Intel Pentium CPU N3700 @ 1.60 GHz x 4 with 4 GB RAM. There the ForTheL text needs a total time of 4 minutes with 3 minutes and 43 seconds used by the prover and the LaTeX document needs a total time of 6 minutes and 6 seconds and 3 minutes 55 seconds by the prover. As one can see, the time needed to parse the LaTeX document is way to high at the moment and needs to be shortened greatly.
...
\noindent First of all, the whole LaTeX parser is really made up on the spot, so it lacks a lot of well needed features, for example being able to process functions as '$abs$' and '$inv$' which have parameters. Also its rigidness makes it really hard to add functionality. This is an expected outcome since the parser was written with its main language ForTheL in mind, which had the LaTeX environment added on top of that, but one could think about writing a parser from scratch, which considers LaTeX as its main input language.\\

\noindent Secondly, we have seen in our example of 'ProdConv', where we had to provide hints that certain numbers are real numbers, that the prover has some problems deducing needed assumptions. Maybe this could handled by enforcing a soft typing system, which tags certain numbers as real numbers or with similar needed assumptions. This probably would help the prover a lot and such a system could also help improving the formalizability of algebraic structures, but one has to think about the theoretic consequences of a typing system. \\

\noindent Furthermore, we would like two more features, that would make formalizing in SAD alot more convenient: On the one hand being able to overload operators, as we already did in our LaTeX formalization, would reduce the number of redundant operators like ' $+'$ ', and on the other hand the ability to create chains of inequalities (and maybe even mixing equalities and inequalities) would result in immensely shorter proofs, because application of transitivity basically require both sides of the inequality to be copied at the moment (for example at the end of 'ConvergentImpBounded').\\
